...
RPR_RENDER_PATH = Path('D:/TestsetScale')
PARAM_STEPS = 20
DIST_FX_SLOT = 1
TREM_FX_SLOT = 2
DLY_FX_SLOT = 3
...
def change_param(CurTr, param, val, fx, fx_slot):
    if fx == 'SlapbackDelay' and (param == 6 or param == 12):
        val = val * 0.5  
...
def create_guitar(track_nr, pitch):
    RPR_InsertTrackAtIndex(track_nr, False)
    CurTr = RPR_GetTrack(0, track_nr)
    RPR_TrackFX_AddByName(CurTr, "Ample Guitar LP.dll", False, -1)
...
def git_bass_keys_drums(pitch):
    create_guitar(0, pitch)
    create_bass(1, pitch)
    #set_vol(1, 0.125)
    create_keys(2, pitch)
    #set_vol(2, 0.25)
...
def fx_one(fx_list, vol_list, mix_name, pitch):
...
def process_files():
    ...
    mix_list = ['G+K+B+D'] #, 'G+K+B+D'] #list of isntrument combinations    ('G', 'G+B', 'G+K', 'G+HHC', 'G+HHO', 'G+SD', 'G+KD', 'G+C', 'G+B+D', 'G+K+D', 'G+K+B', 'G+K+B+D'
    ...
    for mix_id, mix in enumerate(mix_list):
        ...
        GitTr = RPR_GetTrack(0, 0)
        fx_one(fx_list, vol_list, mix_name, pitch_list[0])
        track_count = RPR_CountTracks(0)
