...
DATA_PATH = Path('C:/Desktop/uni/Studienarbeit/Audio/NewDataset/G+K+B+D')    \
    if platform.system() == 'Windows' else os.path.join(os.path.expanduser('~'), 'tmp/NewDataset/G+K+B+D')
...
def estimate(folder_path):
    ...
    for train, val in kf.split(feat_data):
        print(fold_no)
        train_data, train_labels = feat_data[train], feat_labels[train]
        test_data, test_labels = feat_data[val], feat_labels[val]
        label_train, label_test = filenames[train], filenames[val]
    ...
    df_name = str(DATA_PATH) + '/' + folder_path  + '/'  + 'Results' + '/' + 'df_j' + '.pickle'
